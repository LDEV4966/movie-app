...
class Detail extends React.Component {
  ...
  getMovieTrailer = async (MovieTitle) => {
    const {
      ...
      `https://www.googleapis.com/youtube/v3/search?key=${API_KEY}&type=video&part=snippet&maxResults=1&q=${MovieTitle}trailer&order=relevance`
    );
    items.forEach((item) => {
        ...
        <div className="detail-container">
          ...
          <div className="contents">
            <div className="youtube__link">
              {TrailerIsLoading ? (
                <span className="loding__film">Loading...</span>
              ) : (
              ...
              <div className="detail_movie_genres">
                <ul className="movie__genres">
                  {movie.genres.map((genre, itemIndex) => {
                    return (
                      <li className="movie__genre" key={itemIndex}>
                        {genre}
                      </li>
                    );
                  })}
